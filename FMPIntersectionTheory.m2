...
projDeg = (X,Y) -> (
    {* 
        X is an ideal generated by polynomials all of the same degree defining a rational map
        Y is a variety containing X which is the source of the rational map
        k is the projDeg we wish to find (probably k=0)
        this function should return the cardinality of the inverse image of a PP^{m-dim(Y)} plane
    *}
    S := ring(X);
